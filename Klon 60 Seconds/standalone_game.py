...
class SoundManager:
    def __init__(self):
        ...
        self.sounds["beep"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["start"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["timer_warning"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["timer_critical"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["phase_transition"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["game_over"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["victory"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["pickup"] = pygame.mixer.Sound(buffer=bytes(0))
        self.sounds["drop"] = pygame.mixer.Sound(buffer=bytes(0))
...
class UI:
    ...
    def render_menu(self, screen):
        ...
        title = "60 SEKUND KLON" if USE_POLISH else "60 SECONDS CLONE"
        title_text = self.font_large.render(title, True, WHITE)
        title_rect = title_text.get_rect(center=(WIDTH//2, HEIGHT//4))
        screen.blit(title_text, title_rect)

        subtitle = "Remake w Pygame/Python" if USE_POLISH else "A Python/Pygame remake"
        ...
        start_text = "Rozpocznij Grę" if USE_POLISH else "Start Game"
        start_button = Button(
        ...
        quit_text = "Wyjdź" if USE_POLISH else "Quit"
        quit_button = Button(
            WIDTH//2 - BUTTON_WIDTH//2,
    ...
    def render_pause_menu(self, screen):
        ...
        pause_text = self.font_large.render("PAUZA" if USE_POLISH else "PAUSED", True, WHITE)
        pause_rect = pause_text.get_rect(center=(WIDTH//2, HEIGHT//4))
        screen.blit(pause_text, pause_rect)

        resume_text = "Kontynuuj" if USE_POLISH else "Resume"
        ...
        menu_text = "Menu Główne" if USE_POLISH else "Main Menu"
        menu_button = Button(
            WIDTH//2 - BUTTON_WIDTH//2,
            HEIGHT//2 + BUTTON_HEIGHT + 20,
            BUTTON_WIDTH,
    ...
    def render_scavenging_ui(self, screen, time_left, inventory):
        ...
        timer_label_text = "POZOSTAŁO SEKUND" if USE_POLISH else "SECONDS REMAINING"
        ...
        inventory_label = "Przedmioty:" if USE_POLISH else "Items:"
    ...
    def render_game_over(self, screen, days_survived, reason):
        ...
        game_over_text = self.font_large.render("KONIEC GRY" if USE_POLISH else "GAME OVER", True, RED)
        game_over_rect = game_over_text.get_rect(center=(WIDTH//2, HEIGHT//4))
        screen.blit(game_over_text, game_over_rect)

        days_label = "Dni Przetrwane:" if USE_POLISH else "Days Survived:"
        ...
        if USE_POLISH and "survived all 30 days" in reason:
            reason = "Przetrwałeś wszystkie 30 dni! Wygrałeś!"
        elif USE_POLISH and "has died" in reason:
            name = reason.split(" ")[0]
        ...
        restart_text = "Nowa Gra" if USE_POLISH else "New Game"
    ...
    def create_default_events(self):
        ...
        self.events = [
            Event(
                1,
                "Radiation Storm",
                "A radiation storm is approaching. You should prepare your shelter.",
                [
                    {
            ...
            Event(
                2,
                "Strange Noises",
                "You hear strange noises outside your shelter. It could be survivors, animals, or worse.",
                [
                    {
            ...
            Event(
                3,
                "Food Shortage",
                "Your food supplies are running dangerously low.",
                [
                    {
                        "text": "Ration food",
                        "outcome": {
...
class SurvivalPhase:
    ...
    def update(self):
        ...
        if self.active_panel == "main":
            ...
            inventory_text = "Ekwipunek" if USE_POLISH else "Inventory"
            self.buttons.append({
            ...
            next_day_text = "Następny Dzień" if USE_POLISH else "Next Day"
            self.buttons.append({
            ...
            back_text = "Powrót" if USE_POLISH else "Back"
            self.buttons.append({
            ...
            food_text = f"Daj Jedzenie ({self.food})" if USE_POLISH else f"Give Food ({self.food})"
            self.buttons.append({
            ...
            water_text = f"Daj Wodę ({self.water})" if USE_POLISH else f"Give Water ({self.water})"
            self.buttons.append({
            ...
            medical_text = f"Lecz ({self.medical})" if USE_POLISH else f"Treat ({self.medical})"
            self.buttons.append({
    ...
    def render_main_panel(self, screen):
        ...
        status_text = font_medium.render("Family Status:", True, WHITE)
        screen.blit(status_text, (70, 30))

        resource_x = WIDTH - 280
        resource_text = font_medium.render("Resources:", True, WHITE)
        ...
        if self.current_events:
            events_text = font_medium.render("Events:", True, WHITE)
            screen.blit(events_text, (50, 160))
        else:
            no_events_text = font_medium.render("No events today.", True, WHITE)
    ...
    def render_character_panel(self, screen):
        ...
        sanity_label = font_medium.render("Sanity:", True, WHITE)
        ...
        hunger_text = font_medium.render(f"Hunger: {self.selected_character.hunger}%", True, WHITE)
        screen.blit(hunger_text, (50, 290))
        
        thirst_text = font_medium.render(f"Thirst: {self.selected_character.thirst}%", True, WHITE)
        screen.blit(thirst_text, (50, 325))

        status_label = font_medium.render("Status:", True, WHITE)
    ...
    def render_event_panel(self, screen):
        ...
        instruction_text = font_small.render("Choose your action:", True, WHITE)
    ...
    def render_inventory_panel(self, screen):
        ...
        title_text = font_medium.render("Supplies", True, WHITE)
        screen.blit(title_text, (50, 100))
